...
// =====================================================================================================================
// These are the HTTP handlers, one per Azure Function, see the function.json files for the binding & triggers
// =====================================================================================================================
...
func resizerFunctionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	// Step 2. Get the inputBlob trigger data,
	// NOTE! It will be Base64 encoded as it's sent to the handler as part of a JSON payload
	// So let's decode that mess into a byte array
	...
	// NOTE! Secretly json.Marshall will Base64 encode the byte array for us, nice!
	...
	resp.Header().Set("Content-Type", "application/json")
	_, _ = resp.Write(invokeResponseJSON)
}

// Render a fractal and return it as image/png
func fractalFunctionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	fractal := fractals.Fractal{
		...
		Center:       fractals.ComplexPair{R: r, I: i},
		MagFactor:    zoom,
		MaxIter:      iters,
		W:            3.0,
		H:            2.0,
		ImgWidth:     width,
		JuliaSeed:    fractals.ComplexPair{R: 0.355, I: 0.355},
		InnerColor:   "#000000",
		FullScreen:   false,
		ColorRepeats: 2.0,
	}
